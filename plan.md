...
- **预计耗时**: 4-6h
- **分配给**: sub-agent-windows
- **产出物**:
  - 打包：`apps/desktop/dist/灵犀演示-win.exe`（Windows 安装包）
  - 截图：`screenshots/T-3.2/`（Win 端到端）
...
| CLI 在 Windows 上不可用 | daemon 调用 MiniMax CLI 失败 | API provider 兜底（已设计）；Win 端先压测 CLI 兼容性 | sub-agent-windows |
